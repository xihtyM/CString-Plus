...
//
// Special cases for converting fmaxes
//

static to_chars_result
__handle_special_value_fmax(
    char *first,
    char *const last,
    const fmax_t value)
{
    ...
    const fmax_t value)
{
    ...
    const bool sign = value < 0;
    const fmax_t abs_value = sign ? -value : value;
...
static to_chars_result
__handle_u64overflow_fmax(
    char *first,
    char *const last,
    const fmax_t value)
{
    const f32 abs_value = value < 0 ? -value : value;
    if (abs_value < u32_max && abs_value > 1e-5l)
        return (to_chars_result){null, success};
    
    fmax_t mantissa = value;
    i16 exponent = 0;

    while (mantissa >= 10.l || mantissa <= -10.l)
    {
        mantissa /= 10.l;
        exponent++;
    }

    while (mantissa < 1.l && mantissa > -1.l)
    {
        mantissa *= 10.l;
        exponent--;
    }
    ...
    if (mantissa > 9.99999l || mantissa < -9.99999l)
    {
        exponent += mantissa < 0.l ? -1 : 1;
        mantissa = 1.l;
    }
    ...
    to_chars_result res = _fmax_to_chars_raw(first, last, mantissa);
    ...
    *first++ = exp_signed ? '-' : '+';
    ...
    {
        *first++ = '0' + abs_exponent / 1000;
        *first++ = '0' + (abs_exponent / 100) % 10;
        *first++ = '0' + (abs_exponent % 100) / 10;
        *first++ = '0' + abs_exponent % 10;
    }
    ...
    const fmax_t value)
{
    ...
    if ((result = __handle_special_value_fmax(first, last, value)).ptr || result.errc != success)
        return result;
    
    if ((result = __handle_u64overflow_fmax(first, last, value)).ptr || result.errc != success)
        return result;

    return _fmax_to_chars_raw(first, last, value);
}
